...
class RegionOfInterestExtractor:
    ...
    def read_yolo_detection_file(self, image_file):
        ...
        Returns:
            list: A list of dictionaries containing polygon points information.
        """
        ...
        for line in lines:
            ...
            if len(data) >= 5:  # Check for class index and polygon points
                class_index = int(data[0])
                points = list(map(float, data[1:]))  # Remaining points are polygon coordinates
                yolo_data.append({"class_index": class_index, "points": points})

        return yolo_data

    def extract_roi(self, image, yolo_data, class_name):
        """
        Extract region of interest from an image based on YOLO polygon data.
        ...
        for data in yolo_data:
            ...
            if detected_class_index == class_index:
                ...
                points = [(int(x * image.shape[1]), int(y * image.shape[0])) for x, y in zip(points[::2], points[1::2])]
